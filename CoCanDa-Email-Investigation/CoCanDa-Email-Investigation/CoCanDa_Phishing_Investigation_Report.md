...
The original `.eml` file was opened in Notepad++ to inspect the full message headers and delivery path.

![Full email header opened in Notepad++](images/01-email-header-overview.png)

The authentication results showed an SPF failure. The domain `microapple.com` did not authorize `93.99.104.210` to send the message.

![SPF failure and source IP address](images/02-spf-failure.png)

The visible sender was `Bill <billjobs@microapple.com>`, while the `Reply-To` address was `negeja3921@pashter.com`. The different domains were a major phishing indicator.

![From, Errors-To, and Reply-To fields](images/03-sender-reply-to.png)

### 2. Base64 Message Decoding

The encoded email content was decoded with CyberChef. The recovered message claimed responsibility for the abductions, demanded one billion CoCanDs, and instructed the Major to solve the attached puzzle.

![Decoded ransom message in CyberChef](images/04-base64-ransom-message.png)

### 3. Attachment Type Identification

CyberChef exposed the starting bytes `50 4B 03 04`, indicating ZIP-compatible content rather than a standard PDF attachment.

![ZIP magic bytes shown in CyberChef](images/05-zip-magic-bytes.png)

The signature was compared with a file-signature reference to confirm the ZIP format.

![ZIP file-signature reference](images/06-zip-signature-reference.png)

### 4. Archive Extraction

The archive was extracted with 7-Zip. It initially contained the extensionless files `DaughtersCrown` and `GoodJobMajor`.

![DaughtersCrown and GoodJobMajor after extraction](images/07-extracted-files.png)

Further analysis revealed the third file, `Money.xlsx`.

![Extracted files including Money.xlsx](images/08-extracted-files-with-xlsx.png)

### 5. DaughtersCrown File Analysis

HxD showed `FF D8 FF E0` at the start of `DaughtersCrown`.

![DaughtersCrown magic bytes in HxD](images/09-daughterscrown-hex.png)

The bytes matched the JPEG/JFIF file signature.

![JPEG file-signature reference](images/10-jpeg-signature-reference.png)

After adding the correct `.jpeg` extension, the file displayed a crown image.

![Recovered DaughtersCrown JPEG](images/11-daughterscrown-image.png)

### 6. GoodJobMajor File Analysis

The bytes `25 50 44 46` identified `GoodJobMajor` as a PDF document.

![PDF file-signature reference](images/12-pdf-signature-reference.png)

The recovered PDF stated that the CoCanDians were safe, referred to `DaughtersCrown` as proof, and directed the Major to `Money.xlsx`.

![Recovered GoodJobMajor PDF](images/13-goodjobmajor-pdf.png)

### 7. Money.xlsx Analysis

The `50 4B 03 04` signature also matched the Microsoft OOXML format used by `.xlsx` files.

![Microsoft OOXML file-signature reference](images/14-ooxml-signature-reference.png)

The workbook revealed that the ransom demand was a deception and declared the beginning of a war with CoCanDa.

![Message recovered from Money.xlsx](images/15-money-xlsx-message.png)

A Base64-encoded string was found inside the workbook.

![Base64 string recovered from Money.xlsx](images/16-money-xlsx-base64.png)

The final CyberChef decode revealed the location: **The Martian Colony, beside the Interplanetary Spaceport**.

![Final location decoded in CyberChef](images/17-final-location-decoded.png)
